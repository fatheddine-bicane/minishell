...
static t_token	*token_end(t_token *head, t_token *tail)
{
	if (head == NULL)
...
static bool	token_head(t_token **phead, char *src, size_t *current,
		char **err_msg)
{
	t_token	*head;

	head = token_identify_bonus(src, current, err_msg);
	if (head == NULL)
		return (false);
...
bool	token_tail(t_token **ptail, char *src, size_t *current, char **err_msg)
{
	...
	if (ptail == NULL || *ptail == NULL)
		return (false);
	tail = *ptail;
	tail->next = token_identify_bonus(src, current, err_msg);
	if (tail->next == NULL)
		return (false);
